...
void mecanum_basic(float y,float x,float w)
{
	...
	float i=PidIncCtrl(&rf,rightfront,y-x+w*SUM);
	float j=PidIncCtrl(&rr,rightrear,y+x+w*SUM);
	float k=PidIncCtrl(&lf,leftfront,y+x-w*SUM);
	float l=PidIncCtrl(&lr,leftrear,y-x-w*SUM);
	if(i>=0)
	{
